    ...
    <div className="detail-overlay" onClick={onClose}>
      <div className="detail-panel" onClick={e=>e.stopPropagation()}>
        ...
        <div className="detail-body">
          ...
          {(plant.pests||[]).length>0 && <div className="detail-section"><div className="section-head">Common Pests</div>{plant.pests.map(pid=>{const pest=pestMap[pid];if(!pest)return null;return<div key={pid} className="pest-card"><div className="pest-name">{pest.emoji} {pest.name}</div><div className="pest-damage">{pest.damage}</div><ul className="ul-arrow">{(pest.treatments||[]).slice(0,3).map((t,i)=><li key={i}>{t}</li>)}</ul></div>;})}</div>}
        </div>
      </div>
    </div>
  );
